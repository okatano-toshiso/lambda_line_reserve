...
def handler(event, context, db_initializer):
    ...
    try:
        ...
        for line_reserve_data in line_reserves_data:
            ...
            filter_keys = ['id', 'reservation_id', 'name', 'line_id', 'phone_number','check_in', 'check_out', 'room_type', 'count_of_person']
            filter_conditions = {key: line_reserve_data[key] for key in filter_keys if key in line_reserve_data}
            existing_reserve = session.query(LineReserve).filter_by(**filter_conditions).first()
            ...
            #     name=line_reserve_data['name'],
            #     line_id=line_reserve_data['line_id'],
            #     phone_number=line_reserve_data['phone_number']
            # ).first()
            if existing_reserve:
